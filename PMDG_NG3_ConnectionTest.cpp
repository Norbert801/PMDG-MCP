...
//
//  NOTE - add these lines to <MSFS Community Folder>\pmdg-aircraft-737\work\737_Options.ini : 
//
//  [SDK]
...
bool NG3_TaxiLightSwitch = true;
bool NG3_LogoLightSwitch = true;
	...
	case SIMCONNECT_RECV_ID_EVENT:	
		{
			...
			case EVENT_KEYBOARD_A:
				{
					printf("Toggling Taxi Light switch:\n");
					toggleTaxiLightSwitch();
					break;
				}
			case EVENT_KEYBOARD_B:
				{
					printf("Toggling Logo Light switch:\n");
					toggleLogoLightsSwitch();
					break;
				}
			case EVENT_KEYBOARD_C:
				{
					printf("Toggling Flight Director switch\n");
					toggleFlightDirector();
					break;
				}
			case EVENT_KEYBOARD_D:
				{
					printf("Slewing Heading selector\n");
					slewHeadingSelector();
					break;
				}
			}
...
void testCommunication()
{
    ...
    if (SUCCEEDED(SimConnect_Open(&hSimConnect, "PMDG NG3 Test", NULL, 0, 0, 0)))
    {
		...
		// 4) Assign keyboard shortcuts
 		hr = SimConnect_MapClientEventToSimEvent(hSimConnect, EVENT_KEYBOARD_A, "Custom.EventA");
 		hr = SimConnect_MapClientEventToSimEvent(hSimConnect, EVENT_KEYBOARD_B, "Custom.EventB");
 		hr = SimConnect_MapClientEventToSimEvent(hSimConnect, EVENT_KEYBOARD_C, "Custom.EventC");
 		hr = SimConnect_MapClientEventToSimEvent(hSimConnect, EVENT_KEYBOARD_D, "Custom.EventD");
		...
		hr = SimConnect_MapInputEventToClientEvent(hSimConnect, INPUT0, "a", EVENT_KEYBOARD_A);
		hr = SimConnect_MapInputEventToClientEvent(hSimConnect, INPUT0, "b", EVENT_KEYBOARD_B);
		hr = SimConnect_MapInputEventToClientEvent(hSimConnect, INPUT0, "c", EVENT_KEYBOARD_C);
		hr = SimConnect_MapInputEventToClientEvent(hSimConnect, INPUT0, "d", EVENT_KEYBOARD_D);
